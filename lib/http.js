...
HttpClient.prototype.request = function(rurl, data, callback, exheaders, exoptions) {
  ...
  if (rurl.includes('webservices.preprod.kvk.nl') || rurl.includes('webservices.kvk.nl')) {
    options.cert = fs.readFileSync('./keys-data-services/innovatielab-2018.kvk.nl.cer');
    options.key = fs.readFileSync('./keys-data-services/privateKey.pem');
    options.gzip = true
  }
